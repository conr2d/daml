...
class SimulationLogSpec extends FlatSpec with Matchers with TypeCheckedTripleEquals {
  ...
  it should "return correct result for minimal log" in {
    ...
    val expected = SimulationLog(
      simulation = "standardfixtradesimulation",
      scenarios = ScenarioStats(
        "fix-trade.standard",
        maxUsers = 1,
        requestsByType = Map("fix-trade-sync" -> RequestTypeStats.fromRequestStats(request :: Nil))
      ) :: Nil
    )
  ...
  it should "produce correct CSV" in {
    ...
    resultFor("multiple-scenarios").map(_.toCsv).getOrElse(throw new AssertionError()) should ===(
